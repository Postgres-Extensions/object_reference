...
*Ground-truth reference of behavior as currently shipped on master (/root/git/object_reference, branch docs/oid-repair-current-behavior, identical to upstream/master).*

## Overview

**The problem.** PostgreSQL object OIDs are not stable across a `pg_dump`/`pg_restore` or a binary `pg_upgrade` — every object gets a new OID when it's recreated in a fresh database or cluster. Anything that needs to durably reference "this same logical object" across those events can't just store an OID. `object_reference` exists to provide that durable reference: it hands out its own stable, synthetic `object_id` for a tracked object, and is responsible for keeping that `object_id` correctly mapped to whatever the object's *current* real OID actually is.

**The two-table model.** Every tracked object has a row in two tables:
- **`_object_reference.object`** — the durable half. Stores the object's *name* (schema-qualified name path + type + argument types — exactly the triple `pg_identify_object_as_address()`/`pg_get_object_address()` use to convert between a name and an OID). Included in `pg_dump`, so it survives a logical restore intact.
- **`_object_reference._object_oid`** — the disposable half. A cache of the object's last-known live `(classid, objid, objsubid)` OID triple. Deliberately *excluded* from every dump — an OID captured in one database instance means nothing in another — and treated as fully rebuildable from `object`'s stored name via a fresh catalog lookup.

**Getting/creating a tracked identity.** The main entry point, `object__getsert()`, is a get-or-create-and-reconcile operation: given an object's name, it finds or creates the matching `object` row, then checks whether `_object_oid` already has a matching, correct cache row. If the cache row is simply missing, it's (re)created automatically — this is the "self-heal" in the title. If a cache row exists but doesn't match the object's real current OID, that is treated as a serious, unexpected error rather than something to reconcile automatically.

**Three ways the OID cache goes stale, and how well each is actually handled today:**
1. **Logical `pg_dump`/`pg_restore` — handled correctly.** `_object_oid` isn't dumped, but a populated materialized view (`_sentry_mv`, whose defining query is `_repair()`) is marked so its `REFRESH` statement lands in `pg_restore`'s very last phase — by which point every tracked object already has its final, restored-database OID. That `REFRESH` re-derives every object's current OID from its stored name and repopulates `_object_oid` from scratch. Verified end-to-end: every tracked object comes back fully consistent after a restore.
2. **Binary `pg_upgrade` — only partially handled, and the mechanism above actively breaks the upgrade itself.** `pg_upgrade` preserves OIDs for table/index/sequence/view/matview/column/type objects, but *not* for constraints, functions, casts, defaults, or triggers — those silently get new OIDs, leaving `_object_oid` stale for them with nothing today that notices or repairs it. Separately: `pg_extension_config_dump()` (the mechanism behind trick #1) was never designed to be called on a materialized view, and under `pg_upgrade`'s internal binary-upgrade dump mode, that specific combination crashes the upgrade outright (`pg_class heap OID value not set when in binary upgrade mode`) — a bug in how `_sentry_mv` is marked, not a general limitation of materialized views under `pg_upgrade`.
3. **Live corruption, or a real rename via ordinary DDL — actively broken.** One of the three background event triggers (`_etg_fix_identity`) re-derives an object's name from whatever OID is *currently* stored in `_object_oid`, on every single DDL statement in the database, with no check that the stored OID is still correct. Once a cache row has gone stale (e.g. left over from a `pg_upgrade`), the very next unrelated DDL statement anywhere in the database either silently overwrites that tracked object's name with some other real object's identity, or — if the stale OID no longer points at anything — throws an uncaught error that aborts not just that statement but every subsequent DDL statement, database-wide, until the bad row is manually cleaned up.

**Bugs in the code meant to catch/report the above** (independent of which of the three scenarios triggers them):
- `fix_refs()` — meant to scan for and report exactly this kind of inconsistency — has a typo (`r_object` instead of the declared `r_object_v`) in its most relevant branch, so instead of reporting "extraneous ID information," it crashes with an unrelated Postgres error every time that branch would fire.
- `_object_oid__add()`/`_repair()` have no `ON CONFLICT` handling, so re-running them against an `object_id` that's already populated (exactly the `pg_upgrade` case) raises a duplicate-key error rather than reconciling anything.
- Because the drop-tracking event trigger (`_etg_drop()`) unconditionally re-runs this same broken repair logic after *every* `DROP` statement, a single stale row anywhere in the tracked set is enough to make an ordinary, unrelated `DROP` fail and roll back.

**Net assessment.** The happy-path logical dump/restore is the one scenario this code demonstrably gets right end-to-end today. `pg_upgrade` is only correctly handled for a subset of object kinds and has a separate bug that can break the upgrade itself before anything else runs. Live staleness — whether left over from an upgrade or from direct corruption — isn't just unhandled: the code that runs unconditionally in that situation actively makes things worse, either silently corrupting tracked identities or blocking all further DDL database-wide.

Everything below this point is supporting reference material — full function/trigger bodies, exact source citations, and reproduction transcripts backing every claim above, organized by component and then by end-to-end scenario. It exists to let the claims above be checked; it is not required reading to understand the design.
